...
class GlobalCrossAttentionLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dropout=0.0,
        ...
        self.activation = _get_activation_fn(activation)
        self.normalize_before = normalize_before
        self.fusion_layer = nn.Conv2d(in_channels=200, out_channels=100, kernel_size=1, stride=1, padding=0)
        self._reset_parameters()
    ...
    def forward_post(self, tgt, memory,
        ...
        tgt_foreground = self.multihead_attn_foreground(query=self.with_pos_embed(tgt, query_pos),
                                   key=self.with_pos_embed(memory, pos),
                                   value=memory, attn_mask=memory_mask_foreground,
                                   key_padding_mask=memory_key_padding_mask)[0]

         
        tgt_background = self.multihead_attn_background(query=self.with_pos_embed(tgt, query_pos),
                                   key=self.with_pos_embed(memory, pos),
                                   value=memory, attn_mask=memory_mask_background,
                                   key_padding_mask=memory_key_padding_mask)[0]
    ...
    def forward(self, tgt, memory,
                ...
                query_pos: Optional[Tensor] = None):
        if self.normalize_before:
            return self.forward_pre(tgt, memory, memory_mask_foreground, memory_mask_background,
                                    memory_key_padding_mask, pos, query_pos)
        return self.forward_post(tgt, memory, memory_mask_foreground, memory_mask_background,
                                 memory_key_padding_mask, pos, query_pos)
...
class MultiScaleMaskedTransformerDecoderFASeg(nn.Module):
    ...
    def forward(self, x, mask_features, pos_list_2d=None):
        # x is a list of multi-scale feature
        assert len(x) == self.num_feature_levels
        src = []
        pos = []
        ...
        # prediction heads on learnable query features
        outputs_class, outputs_mask, attn_mask_foreground, attn_mask_background = self.forward_prediction_heads(output, mask_features,
        ...
        for i in range(self.num_layers):            
            ...
            level_index = i % self.num_feature_levels
            attn_mask_foreground[torch.where(attn_mask_foreground.sum(-1) == attn_mask_foreground.shape[-1])] = False
            attn_mask_background[torch.where(attn_mask_background.sum(-1) == attn_mask_background.shape[-1])] = False
    ...
    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size):
        ...
        # must use bool type
        # If a BoolTensor is provided, positions with ``True`` are not allowed to attend while ``False`` values will be unchanged.
        attn_mask_foreground = (attn_mask.sigmoid().flatten(2).unsqueeze(1).repeat(1, self.num_heads, 1, 1).flatten(0, 1) < 0.5).bool()
        attn_mask_foreground = attn_mask_foreground.detach()

        attn_mask_background = (attn_mask.sigmoid().flatten(2).unsqueeze(1).repeat(1, self.num_heads, 1, 1).flatten(0, 1) > 0.5).bool()
