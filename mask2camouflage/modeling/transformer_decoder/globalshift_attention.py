...
def hypersphere_attention(
    ...
    B, Nt, E = q.shape
    B, Ns, _ = k.shape
    # normalize q and k to make them have unit embedding vectors
    q = F.normalize(q, p=2.0, dim=-1)
    k = F.normalize(k, p=2.0, dim=-1)
    # use ball kernel to get weights for new means
    # (B, Nt, E) x (B, E, Ns) -> (B, Nt, Ns)
    attn = torch.bmm(q, k.transpose(-2, -1))
    # exp and keppa used to scale the weight
    attn = kappa * attn
...
def hypersphere_attention_forward(
    query: Tensor,
    key: Tensor,
    value: Tensor,
    ...
    if not use_separate_proj_weight:
        q, k, v = ms_in_projection_packed(query, key, value, in_proj_weight, in_proj_bias)
    else:
        ...
        q, k, v = ms_in_projection(query, key, value, q_proj_weight, k_proj_weight, v_proj_weight, b_q, b_k, b_v)
...
class GlobalShiftAttention(nn.MultiheadAttention):
    r"""
    ...
    def __init__(self, embed_dim, num_heads=1, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False,
                 kdim=None, vdim=None, batch_first=False, device=None, dtype=None) -> None:
        factory_kwargs = {'device': device, 'dtype': dtype}
        super(GlobalShiftAttention, self).__init__(embed_dim, num_heads)
    ...
    Outputs:
        ...
        #kappa_weight = self.linear_kappa(self.kappa_weight)
...
class MeanShift_Transformer(nn.Module):
    def __init__(
        self,
        d_model=512,
        nhead=8,
...
class TransformerEncoderLayer(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.self_attn = GlobalShiftAttention(d_model, nhead, dropout=dropout)
...
class TransformerDecoderLayer(nn.Module):
    def __init__(
        ...
        self.multihead_attn = GlobalShiftAttention(d_model, nhead, dropout=dropout)
        # Implementation of Feedforward model
        self.linear1 = nn.Linear(d_model, dim_feedforward)
        self.dropout = nn.Dropout(dropout)
        self.linear2 = nn.Linear(dim_feedforward, d_model)
